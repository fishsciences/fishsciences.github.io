...
When working with acoustically-tagged fish, we often generate a record of each fish’s "encounters" with the autonomous acoustic monitors in an [underwater array](http://californiafishtracking.ucdavis.edu/region.shtml).  If this in no way applies to your work, you can think of an encounter history as a simple set of Bernoulli trials, with successes (`1`s) or failures (`0`s).  These encounter histories are simply the translation of a fish's path into a row of ones and zeros, each corresponding to a positive or negative detection record at a receiver location in the acoustic array.  
...
First, it's easier to work with both the `TagID`s and monitors if they're ordered factors (remember, the `Station` order matters, because we want to see the fish go from upstream to downstream):
